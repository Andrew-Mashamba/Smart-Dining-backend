<x-layouts.app title="Kitchen Display System">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            ...
            <p class="text-gray-600">Food Orders Queue</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <!-- Pending Items -->
            <div class="bg-yellow-50 rounded-lg shadow-lg p-4">
                <h2 class="text-xl font-bold text-yellow-800 mb-4">Pending ({{ $pendingItems->count() }})</h2>
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-lg p-4 shadow">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                ...
                                <span class="ml-2 bg-yellow-100 text-yellow-800 text-xs font-semibold px-2 py-1 rounded">x{{ $item->quantity }}</span>
                            </div>
                        ...
                        <form action="{{ route('kitchen.mark-received', $item->id) }}" method="POST">
                            @csrf
            ...
            <div class="bg-blue-50 rounded-lg shadow-lg p-4">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-lg p-4 shadow border-l-4 border-blue-500">
                        ...
                        <p class="text-sm text-red-600 font-semibold mb-2">Note: {{ $item->special_instructions }}</p>
                        @endif
                        <p class="text-xs text-gray-500 mb-2">Chef: {{ $item->preparedBy->name ?? 'N/A' }}</p>
                        <p class="text-xs text-gray-500 mb-3">Started: {{ $item->updated_at->diffForHumans() }}</p>
                        <form action="{{ route('kitchen.mark-done', $item->id) }}" method="POST">
                            @csrf
